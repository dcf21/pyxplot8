...
int directive_histogram(Dict *command)
 {
  ...
  if (data->Nrows<3) { ppl_error(ERR_NUMERIC, "Histogram construction is only possible on data sets with members at at least three values of x."); return 1; }
  ...
  if (xdata==NULL) { ppl_error(ERR_MEMORY, "Out of memory whilst reading data from input file."); return 1; }
  ...
  if (k<3) { ppl_error(ERR_NUMERIC, "Histogram construction is only possible on data sets with members at at least three values of x."); return 1; }
  ...
  if (output->filename == NULL) { ppl_error(ERR_MEMORY,"Out of memory"); return 1; }
  ...
  if (templist != NULL)
   {
    ...
    if (output->bins == NULL) { ppl_error(ERR_MEMORY,"Out of memory"); return 1; }
    j=0;
  ...
  else
   {
    ...
    if (((xbinmax-xbinmin)/BinWidthDbl + 1.0001) > 1e7) { sprintf(temp_err_string, "The supplied value of BinWidth produces a binning scheme with more than 1e7 bins. This is probably not sensible."); ppl_error(ERR_GENERAL, temp_err_string); return 1; }
  ...
  if (output->binvals == NULL) { ppl_error(ERR_MEMORY,"Out of memory"); free(output->bins); return 1; }
