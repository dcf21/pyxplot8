...
unsigned char MAKE_INLINE ppl_units_DblEqual(double a, double b)
 {
...
unsigned char MAKE_INLINE ppl_units_DblApprox(double a, double b, double err)
 {
  if ( fabs(a-b) > (fabs(err)+1e-50) ) return 0;
  return 1;
 }

void MAKE_INLINE ppl_units_DimCpy(value *o, const value *i)
 {
  int j;
...
void MAKE_INLINE ppl_units_DimInverse(value *o, const value *i)
 {
  int j;
  o->dimensionless = i->dimensionless;
  for (j=0; j<UNITS_MAX_BASEUNITS; j++) o->exponent[j] = -i->exponent[j];
...
int MAKE_INLINE ppl_units_DimEqual(const value *a, const value *b)
 {
  int j;
...
int MAKE_INLINE ppl_units_DimEqual2(const value *a, const unit *b)
 {
  int j;
  for (j=0; j<UNITS_MAX_BASEUNITS; j++) if (ppl_units_DblEqual(a->exponent[j] , b->exponent[j]) == 0) return 0;
  return 1;
...
int MAKE_INLINE ppl_units_UnitDimEqual(const unit *a, const unit *b)
 {
  int j;
  for (j=0; j<UNITS_MAX_BASEUNITS; j++) if (ppl_units_DblEqual(a->exponent[j] , b->exponent[j]) == 0) return 0;
  return 1;
...
unsigned char MAKE_INLINE TempTypeMatch(unsigned char a, unsigned char b)
 {
...
void MAKE_INLINE ppl_units_pow (const value *a, const value *b, value *o, int *status, char *errtext)
 {
  int i;
  double exponent;
  gsl_complex ac, bc;
...
void MAKE_INLINE ppl_units_mult(const value *a, const value *b, value *o, int *status, char *errtext)
 {
  int i;
...
void MAKE_INLINE ppl_units_div (const value *a, const value *b, value *o, int *status, char *errtext)
 {
  int i;
...
void MAKE_INLINE ppl_units_add (const value *a, const value *b, value *o, int *status, char *errtext)
 {
...
void MAKE_INLINE ppl_units_sub (const value *a, const value *b, value *o, int *status, char *errtext)
 {
  o->real = a->real - b->real;
  if ((o != a) && (o != b)) { ppl_units_DimCpy(o,a); o->imag = 0.0; o->FlagComplex=0; }
  if ((a->dimensionless == 0) || (b->dimensionless == 0))
...
void MAKE_INLINE ppl_units_mod (const value *a, const value *b, value *o, int *status, char *errtext)
 {
