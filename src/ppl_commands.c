...
char ppl_commands[] = "\
arrow@2:directive = { from@1 } %fu:x1 ,@n %fu:y1 to@1 %fu:x2 ,@n %fu:y2 { with@1 ( < linetype@5 | lt@2 > %d:linetype ~ < linewidth@5 | lw@2 > %f:linewidth ~ < linestyle@6 | ls@2 > %d:linestyle ~ < colour@1 | color@1 > %s:colour ~ < nohead@2:arrow_style | head@2:arrow_style | twohead@2:arrow_style | twoway@2:arrow_style:twohead > ) } \n\
cd@2:directive = [ < %q:directory | %Q:directory | %S:directory > ]:path\n\
clear@3:directive =\n\
delete@3:directive = [ %d:number ]:deleteno,\n\
edit@2:directive = %d:editno\n\
eps@2:directive = < %q:filename | %Q:filename | %S:filename > ( at@2 %fu:x ,@n %fu:y ~ rotate@1 %fu:rotation ~ width@1 %fu:width ~ height@1 %fu:height )\n\
...
< jpeg@1:directive | jpg@2:directive:jpeg > = < %q:filename | %Q:filename | %S:filename > ( at@2 %fu:x ,@n %fu:y ~ rotate@1 %fu:rotation ~ width@1 %fu:width ~ height@1 %fu:height )\n\
list@2:directive =\n\
load@2:directive = < %q:filename | %Q:filename | %S:filename >\n\
move@3:directive = %d:moveno to@1 %fu:x ,@n %fu:y \n\
...
print@2:directive = [ < %q:string | %fu:expression | %Q:string > ]:@print_list, \n\
pwd@2:directive =\n\
quit@1:directive =\n\
refresh@3:directive =\n\
reset@3:directive =\n\
...
set@2:directive key@1:set_option = < below@2:pos | above@2:pos | outside@1:pos | ( < left@1:xpos | right@1:xpos | xcentre@1:xpos | xcenter@1:xpos:xcentre > ~ < top@1:ypos | bottom@2:ypos | ycentre@1:ypos | ycenter@1:ypos:ycentre > ) > { %fu:x_offset ,@n %fu:y_offset }\n\
...
set@2:directive origin@2:set_option = %fu:x_origin ,@n %fu:y_origin\n\
...
set@2:directive size@1:set_option = < %fu:width | ratio@1 %f:ratio | noratio@1:noratio | square@1:square >\n\
set@2:directive terminal@1:set_option = ( < x11_singlewindow@1:term:X11_singlewindow | x11_multiwindow@5:term:X11_multiwindow | x11_persist@5:term:X11_persist | postscript@1:term:PS | ps@2:term:PS | eps@1:term:EPS | pdf@2:term:PDF | png@2:term:PNG | gif@1:term:GIF | jpg@1:term:JPG | jpeg@1:term:JPG > ~ < colour@1:col:ON | color@1:col:ON | monochrome@1:col:OFF | nocolour@1:col:OFF | nocolor@1:col:OFF > ~ < enlarge@1:enlarge:ON | noenlarge@3:enlarge:OFF > ~ < landscape@1:land:ON | portrait@2:land:OFF > ~ < notransparent@1:trans:OFF | nosolid@1:trans:ON | transparent@1:trans:ON | solid@1:trans:OFF > ~ < invert@1:invert:ON | noinvert@1:invert:OFF > ~ < antialias@1:antiali:ON | noantialias@3:antiali:OFF > )\n\
...
set@2:directive title@2:set_option = < %q:title | %Q:title | %s:title > { %fu:x_offset ,@n %fu:y_offset }\n\
set@2:directive unit@1:set_option = ( display@1 ( < typeable@1:typeable:ON | natural@2:typeable:OFF | notypeable@3:typeable:OFF | nonatural@3:typeable:ON > ~ < prefix@1:prefix:ON | noprefix@3:prefix:OFF > ) ~ scheme@1 < si@2:scheme:SI | cgs@1:scheme:CGS | ancient@1:scheme:ANCIENT | imperial@1:scheme:IMPERIAL | usimperial@1:scheme:US > ~ [ of@1 %s:quantity %s:unit ]:prefered_units, )\n\
set@2:directive width@1:set_option:size = %fu:width\n\
...
text@3:directive = < %q:string | %Q:string | %s:string > ( at@1 %fu:x ,@n %fu:y ~ rotate@1 %fu:rotation ) { with@1 < colour@1 | color@1 > %s:colour }\n\
...
{ < let@3 | set@3 > } %v:varname \\=@n:directive:var_set = { < %fu:value | %Q:value | %q:value > }\n\
";
