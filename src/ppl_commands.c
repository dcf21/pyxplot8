...
< spline@3:directive | linear@3:directive | loglinear@3:directive | polynomial@3:directive > = [ \\[@n { { < %f:min | \\*@n:minauto > } < :@n | to@n > { < %f:max | \\*@n:maxauto > } } \\]@n ]:@range_list %v:fit_function \\()@2 < %q:filename | %Q:filename | %S:filename > ( every@1 [ { %d:every_item } ]:every_list: ~ index@1 %f:index ~ select@1 %E:select_criterion ~ smooth@1 %f:smooth ~ using@1 { < rows@1:use_rows | columns@1:use_columns > } [ %E:using_item ]:using_list: )\n\
