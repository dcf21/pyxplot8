...
int dviGetTFM(dviFontDetails *font)
 {
  ...
  if (DEBUG) { sprintf(temp_err_string, "Font file %s: TFM path: %s\n", font->name, TFMpath); ppl_log(temp_err_string); }
  ...
  if (PFApath != NULL)
  ...
  else
   {
    ...
    snprintf(PFApath, SSTR_LENGTH, "%s%s.pfa", settings_session_default.tempdir, font->name);
...
dviTFM *dviReadTFM(FILE *fp)
 {
  ...
  lh--;
  if (DEBUG) { sprintf(temp_err_string, "TFM: lh now %d\n", lh); ppl_log(temp_err_string); }
  if (lh>10)
   {
    int len;
    ReadUChar(fp, &len);
    if (DEBUG) { sprintf(temp_err_string, "TFM: Coding length: %d\n", len); ppl_log(temp_err_string); }
    if (len>39)
     {
      ppl_error(ERR_GENERAL,"Malformed DVI header!  coding len>40!");
      len=39;
     }
    for (i=0; i<39; i++)
     {
  ...
  if (lh>5)
   {
    ...
    if (len>19)
     {
      ppl_error(ERR_GENERAL,"Malformed DVI header!  coding len>19!");
      len=19;
     }
    for (i=0; i<19; i++)
     {
  ...
  if (DEBUG) { sprintf(temp_err_string, "TFM: coding:%s: family:%s: lh now %d\n", tfm->coding, tfm->family, lh); ppl_log(temp_err_string); }
  if (lh>0)
   {
    int temp;
    ReadUChar(fp, &temp);
  ...
  for (i=0; i<Nchars; i++)
   {
    int j;
    int t[4];
    for (j=0; j<4; j++) ReadUChar(fp, t+j);
  ...
  for (i=0; i<tfm->nw; i++) tfm->width[i]  = ReadFixWord(fp);
  for (i=0; i<tfm->nh; i++) tfm->height[i] = ReadFixWord(fp);
  for (i=0; i<tfm->nd; i++) tfm->depth[i]  = ReadFixWord(fp);
  for (i=0; i<tfm->ni; i++) tfm->italic[i] = ReadFixWord(fp);
  ...
  for (i=0; i<tfm->nl; i++)
   {
    int j;
    ReadUChar(fp, &j); (tfm->ligKern+i)->skip_byte = j;
    ReadUChar(fp, &j); (tfm->ligKern+i)->next_char = j;
    ReadUChar(fp, &j); (tfm->ligKern+i)->op_byte   = j;
    ReadUChar(fp, &j); (tfm->ligKern+i)->remainder = j;
   }

  // Read the kern table
  tfm->kern = (double *)lt_malloc(tfm->nk*sizeof(double));
  for (i=0; i<tfm->nk; i++) tfm->kern[i] = ReadFixWord(fp);
  ...
  for (i=0; i<tfm->ne; i++)
   {
    int j;
    ReadUChar(fp, &j); (tfm->extensibleRecipe+i)->top = j;
    ReadUChar(fp, &j); (tfm->extensibleRecipe+i)->mid = j;
    ReadUChar(fp, &j); (tfm->extensibleRecipe+i)->bot = j;
    ReadUChar(fp, &j); (tfm->extensibleRecipe+i)->rep = j;
   }
