...
void eps_eps_RenderEPS(EPSComm *x)
 {
  ...
  if ((x->current->clip) && (GotBBox))
   {
    fprintf(x->epsbuffer, "%f %f %f %f rect  %% Set up clipping path\n", bb_left, bb_bottom, bb_right, bb_top);
    fprintf(x->epsbuffer, "clip newpath      %% Set the clipping path\n");
   }
...
void eps_eps_ExtractBBox(FILE *in, double *bl, double *bb, double *br, double *bt, unsigned char *GotBox)
  ...
  *GotBox = 0;
  ...
  while ((j=fgetc(in))!=EOF)
   {
    if (BBoxStr  [bbox_i  ] == j) bbox_i  ++;
    if (HRBBoxStr[hrbbox_i] == j) hrbbox_i++;
