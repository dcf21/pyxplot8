...
char *ppl_glob_oneresult(char *filename)
 {
  ...
  if (ppl_glob_SpecialFilename(filename))
   ...
   {
    if (wordexp(filename, &WordExp, 0) != 0) { sprintf(temp_err_string, "Could not glob filename '%s'.", filename); ppl_error(ERR_FILE, -1, -1, temp_err_string); return NULL; }
    ...
    if (glob(WordExp.we_wordv[0], 0, NULL, &GlobData) != 0) { sprintf(temp_err_string, "Could not glob filename '%s'.", WordExp.we_wordv[0]); ppl_error(ERR_FILE, -1, -1, temp_err_string); wordfree(&WordExp); return NULL; }
...
ppl_glob *ppl_glob_allresults(char *filename)
 {
  ...
  if (ppl_glob_SpecialFilename(filename))
   ...
   {
    if (wordexp(filename, &output->WordExp, 0) != 0) { sprintf(temp_err_string, "Could not glob filename '%s'.", filename); ppl_error(ERR_FILE, -1, -1, temp_err_string); return NULL; }
    ...
    if (glob(output->WordExp.we_wordv[0], 0, NULL, &output->GlobData) != 0) { sprintf(temp_err_string, "Could not glob filename '%s'.", output->WordExp.we_wordv[0]); ppl_error(ERR_FILE, -1, -1, temp_err_string); wordfree(&output->WordExp); return NULL; }
...
char *ppl_glob_iterate(ppl_glob *x)
 {
  ...
  if (x->WordExpSet && (x->iw < x->WordExp.we_wordc))
   {
    if (glob(x->WordExp.we_wordv[x->iw], 0, NULL, &x->GlobData) != 0) { sprintf(temp_err_string, "Could not glob filename '%s'.", x->WordExp.we_wordv[x->iw]); ppl_error(ERR_FILE, -1, -1, temp_err_string); x->GlobDataSet = 0; return NULL; }
    x->GlobDataSet = 1;
