...
%
% Copyright (C) 2006-2010 Dominic Ford <coders@pyxplot.org.uk> 2009-2010 Ross
% Church
%
% $Id$
%
...
variables in PyXPlot, and the user can freely re-define them without this being
an error.
...
\begin{verbatim} f()    = pi f(x)   = x*sin(x) f(x,y) = x*y \end{verbatim}
...
range of values of $x$ for which $F(x)$ is defined, as above, will result in
error messages being returned that the function could not be evaluated at all
...
x = \exp(a+i\theta).
\end{displaymath}
However, it also has some disadvantages: some interesting quantities such as
radiative fluxes per steradian are measured per unit angle or per unit solid
angle, and the SI system offers no way to dimensionally distinguish these from
one another or from quantities with no angular dependence.

\phantom{xxx}
On balance, we have decided that it is more useful if PyXPlot {\it does}
consider the radian to be a base unit, so that it can distinguish such
quantities. The unfortunate consequence of this is that first equation above to
be rewritten as:
\begin{displaymath}
...
statement overrides such behaviour.
...
Day number. For example, midnight in 1st~January 2000 (Gregorian calendar) was
$2451544.5$, and noon on the same day was $2451545.0$.
...
algebraic expression {\tt (y-x)*unit(day)}. In the following example, we
demonstrate its use to calculate the time elapsed between the traditional date
for the foundation of Rome by Romulus and Remus in 753\,{\footnotesize BC} and
that of the deposition of the last Emperor of the Western Empire in
{\footnotesize AD}\,476:
