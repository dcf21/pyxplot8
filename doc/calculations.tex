...
\example{ex:funcsplice}{The use of function splicing to calculate the Fibonacci numbers}{
In this example, we use of function splicing to calculate the Fibonacci numbers in an iterative and highly inefficient way, using the knowledge that the zeroth and first Fibonacci numbers are both one, and that all subsequent numbers are the sums of their two immediate predecessors:

\noindent {\tt fib(x) = 1}\newline
\noindent {\tt fib(x)[2:] = fib(x-1) + fib(x-2)}\newline
\noindent {\tt plot [0:8] fib(x)}\newline

We compare the result against Binet's formula for the Fibonacci numbers,

\begin{displaymath}
f(x) = {\psi^x - (1-\psi)^x}{\sqrt{5}},
\end{displaymath}

\noindent where $\psi=1+\sqrt{5}/2$ is the golden ratio.
...
The \indcmdt{solve} can be used to solve simple systems of
...
\example{ex:eqnsolve}{An example}{
In this example,.
}
