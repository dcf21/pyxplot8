...
on data before it is plotted, and so in this chapter we describe the
mathematical environment which PyXPlot provides for manipulating and performing
calculations. For simplicity, most of the examples in this chapter perform
single calculations on the values of variables, displaying the results using
the {\tt print} command, essentially using PyXPlot as a desktop calculator; we
will show in subsequent chapters examples of the use of PyXPlot's mathematical
...
by the numerical value $x$ of the dummy variable at the point where the given
...
Julian Day (JD) numbers, which count the number of days ellapsed since noon on
1st~January 4713\,{\footnotesize BC} in the Julian Calendar. The time of day is
represented as the fractional part of the Julian Day number. For example,
midnight in 1st~January 2000 (Gregorian calendar) was $2451544.5$, and noon on
the same day was $2451545.0$.
...
time\_juliandate()} is used to convert from a calendar date to a Julian Day
number. It takes six inputs: the year, the month number (1--12), the day of the
month (1--31), the hour of day (0--24), the number of minutes (0--59), and the
number of seconds (0--59). To enter dates before 1\,{\footnotesize AD}, a year
of~$0$ should be passed to indicate 1~{\footnotesize BC}, $-1$ should be passed
to indicate the year 2~{\footnotesize BC}, and so forth.
\begin{verbatim}
pyxplot> time_juliandate(2000,1,1,0,0,0)
2451544.5
\end{verbatim}
...
Thus, inputed dates between 2nd~September and 14th~September 1752 return
errors, since they days never occured in the British calendar. This behaviour may be changed using the \indcmdt{set calendar}, which offers a choice of nine different calendars listed in Table~\ref{tab:calendars}.
...
Islamic &
Use the Islamic (Muslim) calendar. Note that the Islamic calendar is undefined prior to 1st~Muharram 0\,{\footnotesize AH}, corresponding to 18th~July 622\,{\footnotesize AD}. \\
Julian &
...
dates, the function {\tt time\_string()} should be used, which takes two
inputs, the second of which is optional. The first is the Julian Day number to
be converted, and the second is a string which controls the format of the
output.  If no format string is provided, then the format\newline
...
Several functions are provided for returning the various parts of the calendar
date associated with any given Julian Day number. All of these functions, with
the exception of {\tt time\_\-dayweekname()} and {\tt time\_\-monthname()} take
a single input, which should be a Julian Day number:

\noindent
\setlength{\extrarowheight}{2mm}
\begin{longtable}{p{4cm}p{7.3cm}}
{\tt time\_daymonth()    }\dotfill & returns the day of the calendar month. \\
{\tt time\_dayweekname() }\dotfill & takes two arguments, the first of which should be a Julian Day number, and the second of which should be a number $l$ indicating the number of characters to return. Returns the name, truncated to $l$ characters, of the day of the week. If $l<1$, the full name is returned. \\
{\tt time\_dayweeknum()  }\dotfill & returns the number (1=Monday--7=Sunday) of the day of the week. \\
{\tt time\_hour()        }\dotfill & returns the hour of the day. \\
{\tt time\_min()         }\dotfill & returns the minutes component of the time. \\
{\tt time\_monthname()   }\dotfill & takes two arguments, the first of which should be a Julian Day number, and the second of which should be a number $l$ indicating the number of characters to return. Returns the name, truncated to $l$ characters, of the calendar month. If $l<1$, the full name is returned. \\
{\tt time\_monthnum()    }\dotfill & returns the number (1-12) of the calendar month. \\
{\tt time\_sec()         }\dotfill & returns the seconds component of the time. \\
{\tt time\_year()        }\dotfill & returns the calendar year number. \\
\end{longtable}
...
Finally, the function {\tt time\_now()}, which takes no arguments, returns the
Julian Day number corresponding to the current system clock time, as in the
following example:
...
The Russian novelist Leo Tolstoy was born on 28th August 1828 and died on 7th
November 1910 in the Russian calendar. What dates do these correspond to in
the Western calendar?
\nlscf
\noindent {\tt pyxplot> set calendar in russian out british}\newline
\noindent {\tt pyxplot> birth = time\_julianday(1828, 8,28,12,0,0)}\newline
\noindent {\tt pyxplot> death = time\_julianday(1910,11, 7,12,0,0)}\newline
\noindent {\tt pyxplot> print time\_string(birth)}\newline
\noindent {\tt Tue 1828 Sep 9 12:00:00}\newline
\noindent {\tt pyxplot> print time\_string(death)}\newline
