...
\funcdef{conditional($a,b,c$)}{returns either $b$, if $a$ is true, or $c$ otherwise. Arguments $b$ and $c$ may be complex, may have any physical dimensions, or may be strings. Argument $a$ must be a dimensionless real number.}
...
\funcdef{random\_\-chisq($\mu$)}{returns a random sample from a $\chi$-squared distribution with $\mu$ degrees of freedom, where $\mu$ must be a real positive dimensionless integer.}
