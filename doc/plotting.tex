...
which represent the $x$- or $y$-coordinates of the positions of each point,
...
$5^{+2}_{-3}$ instead of $5\pm2$, except that the absolute values of~2 and~7
should be given in the supplied data in place of $-3$ and $+2$:
\begin{itemize}
...
\item \indpst{histeps}
\end{itemize}
Like the {\tt points} plot style, each of these styles takes only two columns
of data as input, containing the $x$- and $y$-coordinates of each \datapoint.
...
are marked by red crosses.  These plot styles differ in their placement of the
...
of arrows that they draw: \indpst{arrows\_head} draws an arrow head on each
arrow at the point $(x_2,y_2)$; \indpst{arrows\_nohead} draws simple lines
without arrow heads on either end; \indpst{arrows\_twohead} draws arrow heads
on both ends of each arrow.
...
\begin{itemize}
\item takes 3 columns
\item set samples grid
\item if datafile, use set samples interpolate
\item set colmap
\item set crange
\item --> renormalise --> reverse
\item set log c1
\item set colkey
\end{itemize}
...
\noindent{\tt set samples grid 200x200}\newline
\noindent{\tt set c1range[-pi:pi]}\newline
\noindent{\tt set c1format "\$\%s$\backslash$pi\$"\%(x/pi)}\newline
...
\noindent{\tt set nokey}\newline
\noindent{\tt set size 8 square}\newline
\noindent{\tt set samples grid 200x200 interpolate MonaghanLattanzio}\newline
\noindent{\tt plot [-2:2][-2:2] '--' with colourmap}\newline
\noindent{\tt -1 -1  1}\newline
\noindent{\tt -1  1 -1}\newline
...
\centerline{\includegraphics[width=8cm]{examples/eps/ex_quadrapole}}
...
numerical implementations of the above iteration, in the absense of any better
...
\begin{itemize}
\item set contour
\item contour labelling
\item different linetype \$1 algebra
\end{itemize}

\noindent{\tt set nokey}\newline
\noindent{\tt set size 8 square}\newline
\noindent{\tt plot [-10:10][-10:10] x**3/20+y**2 with colourmap, $\backslash$}\newline
\noindent{\tt \phantom{xxxxxxxxxxxxxxxxxxxxxx}x**3/20+y**2 with contours col green}
...
\noindent{\tt set zlabel "\$|$\backslash$mathrm\{log\}(z)|\$"}\newline
\noindent{\tt set key below}\newline
\noindent{\tt set size 8 square}\newline
\noindent{\tt set grid}\newline
...
\noindent{\tt \phantom{xxxxx}fillcol hsb(atan2(\$1,\$2)/(2*pi)+0.5):hypot(\$1,\$2)/30+0.2:(\$3*0.5+0.5)}
\nlscf
The resulting image is shown below:
\nlscf
\begin{center}
