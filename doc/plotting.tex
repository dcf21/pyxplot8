...
In Chapter~\ref{ch:first_steps}, a brief overview of the \indcmdt{plot} was
...
Various modifiers may be used to affect the exact appearance of the output
...
\noindent Not all of the modifiers apply to all of PyXPlot's plot styles. Where
modifiers are applied to plot styles for which they have no defined effect,
this is not an error, but the modifier has no effect.
Table~\ref{tab:style_modifiers} lists which modifiers effect which plot styles,
including, for completeness, all of PyXPlot's plot styles.
...
linewidth}, {\tt pointlinewidth} and {\tt pointsize} modifiers can have their
default values changed as in the following examples:
\begin{verbatim}
set linewidth 1
