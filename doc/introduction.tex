...
typing to produce a first draft result.  At the same time, these commands aim
to take a sufficient range of optional arguments and settings to allow them,
after some tweaking, to produce figures in a wide range of styles, which are
amply neat enough to include in reports, talks or academic journals.

PyXPlot is suitable for a wide-range of data processing and visualisation
tasks. It is a graph-plotting package, but also has facilities for fitting
best-fit curves to data, for numerically solving simple systems of equations,
for converting data files between different formats, and is also a remarkably
powerful desktop calculator which can keep track of the physical dimensions of
variables and convert between different units of measurement -- for example,
between inches and centimetres.

\section{PyXPlot's Heritage: \gnuplot}

Users who have previous experience of \gnuplot\ will rapidly notice that
