...
In this example, we draw a labelled diagram of the interior angles of a triangle, taking the lengths of the three sides of the triangle as inputs. We calculate the positions of the three vertices of the triangle and then label them. We use PyXPlot's automatic handling of physical units to generate the \LaTeX\ strings required to label the side lengths in centimetres and the angles in degrees. We use PyXPlot's {\tt arc} command to draw angle symbols in the three corners of a triangle.\\
...
\noindent {\tt }\newline
\noindent {\tt \# Lengths of three sides of triangle}\newline
\noindent {\tt AB = 4*unit(cm)}\newline
\noindent {\tt AC = 5*unit(cm)}\newline
\noindent {\tt BC = 6*unit(cm)}\newline
}\\{\footnotesize
\noindent {\tt \# Use cosine rule to find interior angles}\newline
\noindent {\tt ABC = acos((AB**2 + BC**2 - AC**2) / (2*AB*BC))}\newline
\noindent {\tt BCA = acos((BC**2 + AC**2 - AB**2) / (2*BC*AC))}\newline
\noindent {\tt CAB = acos((AC**2 + AB**2 - BC**2) / (2*AC*AB))}\newline
}\\{\footnotesize
\noindent {\tt \# Positions of three corners of triangle}\newline
\noindent {\tt Bx = 0*unit(cm)~~~~~~~; By = 0*unit(cm) \# The origin}\newline
\noindent {\tt Cx = Bx + BC~~~~~~~~~~; Cy = By}\newline
\noindent {\tt Ax = Bx + AB*cos(ABC)~; Ay = By + AB*sin(ABC)}\newline
}\\{\footnotesize
\noindent {\tt \# Draw triangle}\newline
\noindent {\tt line from Ax,Ay to Bx,By}\newline
\noindent {\tt line from Ax,Ay to Cx,Cy}\newline
\noindent {\tt line from Bx,By to Cx,Cy}\newline
}\\{\footnotesize
\noindent {\tt \# Draw angle symbols}\newline
\noindent {\tt ArcRad = 4*unit(mm) \# Radius of angle arcs}\newline
\noindent {\tt arc at Bx,By radius ArcRad from~~90*unit(deg)-ABC to~~90*unit(deg)}\newline
\noindent {\tt arc at Cx,Cy radius ArcRad from -90*unit(deg)~~~~~to -90*unit(deg)+BCA}\newline
\noindent {\tt arc at Ax,Ay radius ArcRad from~~90*unit(deg)+BCA to 270*unit(deg)-ABC}\newline
}\\{\footnotesize
\noindent {\tt \# Label lengths of sides}\newline
\noindent {\tt set unit of length cm \# Display lengths in cm}\newline
\noindent {\tt set numeric sigfig 3 display latex \# Correct to 3 significant figure}\newline
\noindent {\tt set fontsize 1.2 \# And in slightly bigger text than normal}\newline
\noindent {\tt TextGap = 1*unit(mm)}\newline
\noindent {\tt text "\%s"\%(BC) at (Bx+Cx)/2,(By+Cy)/2 gap TextGap hal c val t}\newline
\noindent {\tt text "\%s"\%(AB) at (Ax+Bx)/2,(Ay+By)/2 gap TextGap rot~~ABC hal c val b}\newline
\noindent {\tt text "\%s"\%(AC) at (Ax+Cx)/2,(Ay+Cy)/2 gap TextGap rot -BCA hal c val b}\newline
}\\{\footnotesize
\noindent {\tt \# Label angles}\newline
\noindent {\tt set unit of angle degree \# Display angles in degrees}\newline
\noindent {\tt ArcRad2 = 1.4 * ArcRad \# Distance of text from apex of angles}\newline
\noindent {\tt text "\%s"\%(CAB) at Ax+ArcRad2*sin(ABC-BCA),Ay-ArcRad2*cos(ABC-BCA) hal c val t}\newline
\noindent {\tt text "\%s"\%(ABC) at Bx+ArcRad2*cos(ABC/2),By+ArcRad2*sin(ABC/2) hal l val c}\newline
\noindent {\tt text "\%s"\%(BCA) at Cx-ArcRad2*cos(BCA/2),Cy+ArcRad2*sin(BCA/2) hal r val c}\newline
}\\{\footnotesize
\noindent {\tt \# Label points ABC}\newline
\noindent {\tt text "A" at Ax,Ay gap TextGap hal c val b}\newline
\noindent {\tt text "B" at Bx,By gap TextGap hal r val c}\newline
\noindent {\tt text "C" at Cx,Cy gap TextGap hal l val c}\newline
}\\{\footnotesize
\noindent {\tt \# Display diagram}\newline
\noindent {\tt set display ; refresh}\newline
}
...
\centerline{\includegraphics[width=5cm]{examples/eps/ex_triangle.eps}}
}
