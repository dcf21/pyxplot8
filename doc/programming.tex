...
a compact means of inserting conditional into numerical and string expressions
respectively.  Each takes three arguments, the first of which is a truth
criterion to be tested. If the criterion is true, then the function's second
argument is returned as its output. Otherwise, the function's third argument is
returned. This is similar to the behaviour of C's ternary {\tt ?:} operator.
The {\tt conditionalN()} function should be passed two numerical expressions to
select between, whilst the {\tt conditionalS()} function should be passed two
string expressions, as the following examples demonstrate:

{\footnotesize
\noindent{\tt pyxplot> f(x) = conditionalN(x$>$0,x,0)}\newline
\noindent{\tt pyxplot> print "\%s \%s \%s \%s \%s"\%(f(-2),f(-1),f(0),f(1),f(2))}\newline
\noindent{\tt 0 0 0 1 2}\newline
\noindent{\tt pyxplot> x = 2}\newline
\noindent{\tt pyxplot> print "x is \%s"\%(conditionalS(x$>$0,"positive","negative"))}\newline
