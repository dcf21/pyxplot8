...
repetitive tasks.  Loop constructs make it possible to produce similar plots of
many different \datafile s, or to perform calculations in an iterative fashion.
We go on to introduce string variables, which can be useful for auto-generating
titles and labels for graphs which are being produced in batch jobs. Finally,
we introduce a simple framework which automatically re-executing PyXPlot
scripts whenever they change, allowing plots to be automatically regenerated
whenever the scripts used to produce them are modified.
...
complicated structure out of simple algorithms, and so we now use all of
PyXPlot's loop constructs together in a script which produces an image of the
fractal.
\nlnp
...
$|z_n|=2$.
\nlnp
This script makes use of the \indcmdt{rectangle}, which will be introduced in
