...
\begin{verbatim}
arc [at] <x>, <y> radius <r> from <start> to <finish>
    [with {<option>}]
\end{verbatim}
...
plot} command. An example would be:
...
\begin{verbatim}
box at <x>, <y> width <w> height <h> [ rotate <r> ]
    [ with {<option>} ]

box from <x1>, <y1> to <x2>, <y2> [ rotate <r> ]
    [ with {<option>} ]
\end{verbatim}
...
The {\tt break} command may only be used within loop structures, and by default
breaks the smallest loop which contains it. If a larger loop is to be broken,
the {\tt break} command may optionally be followed by the name of the loop to
be broken.
...
\begin{verbatim}
circle [at] <x>, <y> radius <r> [ with {<option>} ]
\end{verbatim}
...
C language.  For example, the following programme will not print the number~2:
...
The {\tt continue} command may only be used within loop structures, and by
default acts upon the smallest loop which contains it. If a larger loop is the
intended target, the {\tt continue} command may optionally be followed by the
name of that loop.
...
\begin{verbatim}
delete <plot number> {, <plot number>}
\end{verbatim}
...
The \indcmdt{foreach} has two uses.  The first is to take a set of filename,
their names produced by expansion of a user-provided expression, and run a set
of commands with a variable set to each filename.  For example, to plot the
data in the set of files whose names end with {\tt .dat}:
...
The \indcmdt{help} provides an hierarchical source of information which
...
The \indcmdt{if} allows conditional execution of commands.  If the {\tt
...
image '<filename>' [at <x>, <y>] [rotate <angle>] [width <width>]
                   [height <height>] [smooth] [notransparent]
...
should be specified using its reference number. For example
...
set origin 8,8} had been executed before it had originally been plotted.


\section{plot}\indcmd{plot}

\begin{verbatim}
plot [{<range>}] ( '<filename>' | <function> )
     [axes <axes>]
     [every <expression> {:<expression>} ]
     [index <value>]
     [select <expression>]
...
In plots which have multiple parallel axes -- for example, an {\tt x}-axis along its
lower edge and an {\tt x2}-axis along its upper edge -- the pair of axes against
...
The \indcmdt{refresh} produces an exact copy of the latest display. This can be
...
\begin{verbatim}
point [at] <x>, <y> [label <string>] [with {<option>} ]
\end{verbatim}
...
In single plot mode, the \indcmdt{replot} causes the most recent plot
...
The \indcmdt{set arrow} causes an arrow to be added to a plot. An example of
its syntax would be:
...
\noindent may be used to add a second horizontal axis to a plot, with default settings. In
general, there is no practical reason to use this command, as a second horizontal axis
...
without a second horizontal axis would be to delete this axis with the following
command:
...
cannot be deleted; a plot must have at least one horizontal and one vertical axis. Instead,
...
standard wildcard characters * and ? may be used.  Then a filter programme
should be specified, along with any necessary options.  This should take the
name of the file to be filtered as the final option on its command line,
immediately following any options specified above.  It should and provide a
suitable PyXPlot data file on standard output for PyXPlot to read.  For
example, to filter all files that end in {\tt .foo} through the {\tt defoo}
filter using the --text option one would type:
...
the default axes (usually the first horizontal and vertical axes).  Conversely, issuing:
...
specified in a range of coordinate systems. These are specified as follows:
