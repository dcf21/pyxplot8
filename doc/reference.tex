...
\subsection{colkey}\indcmd{set colkey}

\begin{verbatim}
set colkey [<position>]
\end{verbatim}

The setting {\tt colkey} determines whether colour scales are drawn along the
edges of plots drawn using the \indpst{colourmap} plot style. Issuing the
command
...
specified after the {\tt set colkey} command, as in the example:
...
\subsection{colmap}\indcmd{set colmap}

\begin{verbatim}
set colmap ( rgb<r>:<g>:<b> |
             hsb<h>:<s>:<b> |
             cmyk<c>:<m>:<y>:<k> )
\end{verbatim}

The setting {\tt colmap} is used to specify the mapping between ordinate values
and colours used by the \indpst{colourmap} plot style. As elsewhere in PyXPlot,
the colour components should be numerical expressions which evaluate to a value
in the range zero to one. Within these expressions, the variable {\tt c} refers
to the renormalised ordinate values supplied as the third column of data,
varying between zero and one. Thus, the following setting, which is the
default, produces a greyscale colour mapping:

\begin{verbatim}
set colmap rgb(c):(c):(c)
\end{verbatim}

More complex colour mappings, which depend upon more than one data value, are
allowed using syntax of the form:

\begin{verbatim}
set colmap hsb(arg($5+i*$6)/pi+0.5):0.5:(c)
\end{verbatim}

\noindent which would use the complex argument of the expression {\tt
\$5+i*\$6} to determine the hue to use. Note that when data values are read
directly in this way, they are not renormalised.
...
The setting {\tt contours} sets the number of contours which are drawn when
using the \indpst{contourmap} plot style. These are evenly spaced -- either
linearly or logarithmically, depending upon the state of the {\tt logscale c}
setting -- between the values specified in the {\tt crange} setting.


\subsection{crange}\indcmd{set crange}

\begin{verbatim}
set crange <range>
\end{verbatim}

The {\tt set crange} command changes the range of ordinate values represented
by different colours in the \indpst{colourmap} plot style, and by contours in
the \indpst{contourmap} plot style. In each case, the colours or contours are
evenly spaced either linearly or logarithmically, depending upon the state of
the {\tt logscale c} setting.  For more details of the syntax of the range
specifier, see the {\tt set xrange} command. The {\tt *} character may be used
to indicate that the scale should automatically be determined to match the data
provided.
...
\subsection{nocolkey}\indcmd{set nocolkey}

\begin{verbatim}
set nocolkey
\end{verbatim}

Issuing the command {\tt set nocolkey} causes plots to be generated with no
colour scale when the \indpst{contourmap} plot style is used.  See the {\tt set
colkey} command for more details.
...
\indpst{surface} plot styles, and when calculating the contours to be plotted
in the \indpst{contourmap} plot style. If a {\tt *} is given in place of either
of the dimensions, then the same number of samples as are specified in {\tt
<value>} are taken.

After the keyword {\tt interpolate}, the method used to interpolated
