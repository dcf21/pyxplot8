...
may be drawn on a multiplot using the arc command.  The {\tt at} modifier
...
{\tt arrow} command. The style of the arrows produced may be specified by
...
There are two different ways to use the {\tt box} command to specify boxes that
...
The {\tt circle} command allows circles to be added to a multiplot.  The
...
\begin{verbatim}
fft <range specifier> [<range specifier> ...] <function>
    of ( '<filename>' | function ) [ using <using specifier> ]

ifft <range specifier> [<range specifier> ...] <function>
    of ( '<filename>' | function )[ using <using specifier> ]
\end{verbatim}

The {\tt fft} command calculates Fourier transforms of \datafile s or functions.
...
The {\tt let} command sets the variable {\tt varname} to equal {\tt value}.
...
The {\tt point} command places a point on the plot at the specified
...
\begin{verbatim}
set arrow <arrow number> from [<coordinate>] <x>,
          [<coordinate>] <y> to [<coordinate>] <x>,
          [<coordinate>] <y> [with <modifier> ]
\end{verbatim}

\begin{verbatim}
<coordinate> = ( first | second | screen | graph |
                  axis<axisnumber>                  )
\end{verbatim}
...
The {\tt axisunitstyle} command controls the style with which the units of
plotted quantities are indicated on plot axes. The {\tt bracketed} option causes
the units to be placed in parentheses following the axis labels, whilst the {\tt
squarebracketed} option using square brackets instead.  The {\tt ratio} option
causes the units to follow the label as a divisor so as to leave the quantity
dimensionless.
...
\begin{verbatim}
set calendar [ (input|output) ] <calendar name>
\end{verbatim}

The {\tt set calendar} command sets the calendar that PyXPlot uses internally to
...
The {\tt set clip} command causes PyXPlot to clip points that extend over the
edge of the plot axes.
...
\begin{verbatim}
set filter "<file name specifier>" "<filter command>"
\end{verbatim}

The {\tt set filter} command allows filter programmes to be specified to allow
PyXPlot to deal with file types that are not in its \datafile\ format.  Firstly
the set of \datafile s that the filter should apply to must be specified;
the standard wildcard characters * and ? may be used.  Then a filter programme
...
suitable PyXPlot data file on standard output for PyXPlot to read.  For example,
to filter all files that end in ``.foo'' through the `defoo' filter using the
...
\begin{verbatim}
set label <label number> '<text>' [<coordinate>] <x>,
                                  [<coordinate>] <y>
                                  [rotate <angle>]
                                  [with colour <colour>]
\end{verbatim}
...
\noindent would place the word `Hello' at plot coordinates $(0,0)$, as measured on the $x$-
...
set numerics [ ( complex | real ) ] [ errors ( explicit | quiet) ]
    [ display ( tex | natural | typeable) ]
    [ sigfig <precision> ]
\end{verbatim}

The {\tt set numerics} command allows various numerical options to be adjusted.

The {\tt complex} option causes complex arithmetic to be carried out; the {\tt
real} option causes all numbers to be real and any calculation that generates
complex numbers will generate an error.

The {\tt errors} option controls how errors are communicated to the user.  The
...
The {\tt display} option changes the output of numbers that are printed to the
terminal.  Setting the option to {\tt typable} causes the numbers to be printed
in a form suitable for pasting back into PyXPlot commands.  {\tt tex} setting
...
The {\tt sigfig} setting changes the number of significant figures to which
...
The {\tt set seed} command allows the seed used to generate random numbers to be
set.  This allows repeatable sequences of random numbers to be generated in
order to calculate re-producable pseudo-random numbers to be generated.
...
\begin{verbatim}
set trange <range specifier>
\end{verbatim}

The {\tt set trange} command changes the range of the independent {\tt t}
variable used for parametric plots.  For more details of the syntax of the range
specifier, see the {\tt set xrange} command.

\subsection{unit}\indcmd{set unit}

\begin{verbatim}
set unit [ of <dimension> <unit> ]
         [ scheme <unit scheme> ]
         [ preferred <unit> ]
         [ nopreferred <unit> ]
         [ display ( full | abbreviated ) ]
\end{verbatim}

The {\tt set unit} command controls how units are used to display numbers within
PyXPlot.  To change the unit used to display a particular dimension of quantity
the {\tt set unit of} form should be used.  For example, the following command
will cause all lengths to be displayed in inches:

\begin{verbatim}
set unit of length inch
\end{verbatim}

The {\tt set unit preferred} command allows units that should be used
preferentially to be specified; this generalises the {\tt set unit of} command
to compound units.  Issuing {\tt set unit nopreferred} will undo this command.
The {\tt set unit display} command specifies whether the full or abbreviated
names of units should be displayed.
...
\begin{verbatim}
set x[<axisnumber>]format ( auto | '<format>' )
...
\begin{verbatim}
set x[<axisnumber>]range <range specifier>
\end{verbatim}
...
If this ordering is not the desired ordering, then the {\tt swap} command may
