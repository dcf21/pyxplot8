...
interchangeably with single quotes. In case where there is likely to be any
...
points on the arc are the distance given following the {\tt radius} modifier.
...
origin with radius $2\,\mathrm{cm}$.  The usual with modifiers for lines may be
passed; if the {\tt fillcolour} modifier is specified then the arc will be
filled to form a pie-chart slice.
...
be deleted and undeleted subsequently with the {\tt delete} and {\tt undelete}
...
following the \indmodt{with} keyword in the \indcmdt{arrow} as in the {\tt
plot} command. The following example would draw a bidirectional blue arrow:
...
canvases.  The position of the box may be specified in one of two ways.  In the
first, the coordinates of one corner of the box are specified, along with the
width and height. If both the width and height are positive then the
...
\noindent The second syntax allows two pairs of coordinates to be specified.
PyXPlot will then draw a rectangular box with opposing corners at the specified
...
The \indcmdt{break} breaks out of {\tt do}, {\tt while}, {\tt for} or {\tt
foreach} loops.  Execution resumes at the statement following the end of the
loop. To demonstrate this, the following example would only print the numbers~1
and~2:
...
If several loops are nested, the {\tt break} and {\tt continue}
...
The \indcmdt{circle} allows circles to be drawn on multiplot canvases.  The
...
\noindent would draw a red circle filled in blue centred $2\,\mathrm{cm}$ above
and to the right of the origin, with unit radius.
...
In multiplot mode, the \indcmdt{clear} removes all current plots, arrows and
text objects from the working multiplot canvas. Outside of multiplot mode, it
is not especially useful; it removes the current plot to leave a blank canvas.
...
The \indcmdt{continue} may be used within {\tt for}, {\tt foreach}, {\tt do}
and {\tt while} loops.  It causes PyXPlot to skip the remainder of the current
iteration of the loop, in an analogous manner to the continue statement in the
C language.  For example, the following script will not print the number~2:
...
If several loops are nested, the {\tt break} and {\tt continue} statements only
act upon the innermost loop. If either statement is encountered outside of a
loop structure, an error results. Optionally, the {\tt for}, {\tt foreach},
{\tt do} and {\tt while}, may be supplied with a name for the loop, prefixed by
...
their reference numbers. The example
...
The \indcmdt{do} sets up a loop, in a similar manner to the {\tt while}
...
Transforms can be calculated in multiple dimensions.  For each dimension of your
function that you wish to transform a range specification must be provided, in
the form
...
After the range(s) a function name should be provided.  A function of this name
will be generated to represent your transformed data.  Note that this function
is in general complex: you can enable complex numerics with the {\tt set
numerics complex} command.  You can use the {\tt using}, {\tt index}, {\tt
every} and {\tt select} statements to specify how data will be selected from
your function or \datafile\ in an analogous manner to the {\tt plot} command.
...
parameters associated with the best-fitting functional form.
...
The \indcmdt{for} executes a set of commands with a specified variable taking a
different value each time.  In PyXPlot's implementation the value of the
variable increases by a fixed value {\tt step} each time, which may be negative
and is equal to unity if not specified.  The {\tt end} value is not included in
the squence.  The following example prints the squares of the first five
natural numbers:
...
The \indcmdt{foreach} can be used to iteratively run a block of commands in one
of two ways.  In the first case, it takes a set of filenames or filename
wildcards, and executes the block of commands once for each supplied filename,
with a string variable set to each filename in succession.  For example, to
plot the data in the set of files whose names end with {\tt .dat}:
...
provided by the user and executes the block of commands once for each value,
with a variable set to each value in succession.  For example, the following
script would plot normal distributions of three different widths:
...
particular topic, type {\tt help} followed by the name of the topic, as in the
following example:
...
further words to the end of your help request, as in the example:
...
The \indcmdt{histogram} takes a \datafile\ and counts the number of points
...
The \indcmdt{history} outputs the current command-line history to the
...
The \indcmdt{if} allows conditional execution of commands.  If and only if the
{\tt criterion} is satisfied, the list of commands are executed.  It is also
possible to have one or more subsequent \indcmdt{else if} statements, which
have their own criteria which are only considered if the first criterion is not
true.  Finally, an \indcmdt{else} statement precedes a set of commands that are
only executed if none of the preceding statements are true.  The following
example illustrates a chain of {\tt else if} clauses:
...
multiplot from files on disk.  The graphical image may be in bitmap, gif jpeg
or png format; the file type of the image is automatically detected. The {\tt
at} modifier can be used to specify where the bottom-left corner of the image
should be placed; if it is not, then the image is placed at the origin. The
{\tt rotate} modifier can be used to rotate the image by any angle, measured in
degrees counter-clockwise.  The {\tt width} or {\tt height} modifiers can be
used to specify the width or height with which the image should be rendered;
both should be specified in centimetres. If neither is specified then the image
will be rendered with the native dimensions specified within the metadata
present in the image file (if any). If both are specified, then the aspect
ratio of the image is changed.

The keyword {\tt smooth} may optionally be supplied to cause the pixels of the
image to be interpolated\footnote{Many commonly-used postscript display
...
interpolate ( linear | loglinear | polynomial | spline )
       [ <range> ] <function name>"()" '<filename>'
       [ every <expressions> {:<expression>} ]
       [ index <value> ]
       [ select <expression> ]
       [ using <expression>  {:<expression>} ]
\end{verbatim}

The \indcmdt{interpolate} generates a special function which interpolates a
set of data points supplied from a \datafile.  Four types of interpolation are
available; linear, log-linear, polynomial and spline.  Log-linear interpolation
is equivalent to linear interpolation in a log-log space.  For example:
...
canvas, giving their reference numbers and the commands used to produce them.
...
files matching the given wildcard are executed, as in the example:
...
The \indcmdt{maximise} takes a user-supplied expression and finds its
...
The \indcmdt{minimise} takes a user-supplied expression and finds its
...
should be specified using its reference number. The example
...
The {\tt axes} modifier may be used to specified which axes data should be
...
The following example would plot data using the {\tt y}-axis as the ordinate
axis and the {\tt x}-axis as the abscissa axis:
...
By default, PyXPlot generates its own entry in the legend for each dataset
plotted.  The user can override this default setting by using the {\tt title}
modifier. The following example labels a dataset as `Dataset~1':
...
of the words which may be used in place of {\tt lines}: {\tt arrows\_head},
{\tt arrows\_nohead}, {\tt arrows\_twohead}, {\tt boxes}, {\tt dots}, {\tt
FilledRegion}, {\tt fsteps}, {\tt histeps}, {\tt impulses}, {\tt lines}, {\tt
LinesPoints}, {\tt LowerLimits}, {\tt points}, {\tt stars}, {\tt steps}, {\tt
UpperLimits}, {\tt wboxes}, {\tt XErrorBars}, {\tt XErrorRange}, {\tt
XYErrorBars}, {\tt XYErrorRange}, {\tt XYZErrorBars}, {\tt XYZErrorRange}, {\tt
XZErrorBars}, {\tt XZErrorRange}, {\tt YErrorBars}, {\tt YErrorRange}, {\tt
YErrorShaded}, {\tt YZErrorBars}, {\tt YZErrorRange}, {\tt ZErrorBars}, {\tt
ZErrorRange}. In addition, {\tt lp} and {\tt pl} are recognised as
abbreviations for {\tt linespoints}; {\tt errorbars} is recognised as an
abbreviation for {\tt yerrorbars}; {\tt errorrange} is recognised as an
abbreviation for {\tt yerrorrange}; and {\tt arrows\_twoway} is recognised as
an alternative for {\tt arrows\_twohead}.
...
The \indcmdt{print} outputs the value of a mathematical expression to the
...
\noindent would print the value of the variable {\tt a}.
...
to produce second copies of multiplot canvases in different graphical formats
selected using the {\tt set terminal} command.
