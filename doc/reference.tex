...
may be drawn on a multiplot using the \indcmdt{arc}.  The {\tt at} modifier
specifies the coordinates of the centre of curvature, from which all points on
the arc are the distance given following the {\tt radius} modifier.  The angles
{\tt start} and {\tt finish}, measured clockwise from the vertical, control
where the arc begins and finishes.  For example, the command

\begin{verbatim}
arc at 0,0 radius 2 from 90 to 270
\end{verbatim}

\noindent would draw a semi-circle below the $x$-axis, centred on the origin
with radius two.  The usual with modifiers for lines may be passed; if the {\tt
fillcolour} modifier is specified then the arc will be filled to form a
pie-chart slice.
...
\indcmdt{arrow}. The style of the arrows produced may be specified by
following the \indmodt{with} keyword with one of the style keywords
\indkeyt{nohead}, \indkeyt{head} (default) or \indkeyt{twohead}. In addition,
keywords such as \indkeyt{colour}, \indkeyt{linewidth} and \indkeyt{linetype}
have the same syntax and meaning after the \indmodt{with} keyword as in the
{\tt plot} command. An example would be:
...
There are two different ways to use the \indcmdt{box} to specify boxes that
are to be drawn onto a multiplot.  In the first syntax, the coordinates of one
corner of the box are specified, along with the width and height.  If both width
and height are positive then the coordinates are taken to be those of the
bottom left-hand corner of the box.  If a rotation angle is specified then the
box is rotated about the specified corner.  The {\tt with} options allows line
and fill types to be specified.
...
The \indcmdt{circle} allows circles to be added to a multiplot.  The
coordinates of the circle's centre and its radius are specified.  Additional
options may be passed following the {\tt with} modifier that change the line and
fill types.  For example

\begin{verbatim}
circle at 2,2 radius 1 with colour red fillcolour blue
\end{verbatim}

\noindent would draw a red circle filled in blue centred 2~cm above and to the
right of the origin, with unit radius.
...
In multiplot mode, the \indcmdt{clear} removes all current plots, arrows
...
The \indcmdt{delete} is part of the multiplot environment; it removes
...
Ellipses may be placed on multiplot canvases using the \indcmdt{ellipse}. The shape
...
The \indcmdt{eps} inserts an image onto the current multiplot from an
...
The \indcmdt{image} inserts a graphical image into the current multiplot
...
The {\tt image} command is often useful in multiplot mode, allowing images to
be combined with plots, text labels, etc.
...
The \indcmdt{list} outputs a listing of all of the items on a multiplot,
...
The \indcmdt{move} is part of the multiplot environment; it can be used to
...
The \indcmdt{plot} is the main workhorse command of PyXPlot, which is used
...
The \indcmdt{refresh} produces an exact copy of the latest display. This can
...
The \indcmdt{point} places a point on the plot at the specified
coordinates.  It is equivalent to plotting a \datafile\ continaing a single
datum.  If an optional {\tt label} is specified then the text string provided is
placed next to the point.  The {\tt with} modifier may be used to change the
point type, colour, etc.
...
The {\tt binorigin} setting changes the position on the ordinate axis from whence
...
or replotted, the whole multiplot is redrawn to show the change. This can be a
time-consuming process on large and complex multiplots. For this reason, the
{\tt set nodisplay} command is provided, which stops PyXPlot from producing any
graphical output. The {\tt set display} command can subsequently be issued to
...
on the $x$- and $y$-axes.  The tag {\tt 1} immediately following the {\tt
...
of the label are measured relative to the first $x$- and $y$-axes, but can be
...
at position $(3,5)$ centimetres on the multiplot page. Alternatively, either of
the coordinates may be specified as quantities with physical units of length,
such as {\tt unit(35*mm)}.  The {\tt set origin} command is of little use
outside of multiplot mode.
...
The \indcmdt{text} is used to add strings of text to a multiplot. An example
would be:
...
The \indcmdt{undelete} is part of the multiplot environment; it can be used
