...
the two-dimensional equivalent of stepwise interpolation -- and inverse-square
...
argument space as weights. These are selected as follows:
...
Finally, data can be imported from graphical images in bitmap ({\tt .bmp}) format.
...
the mapping between ordinate values and colours. Note that such scales are only
ever drawn when the \indpst{colourmap} plot style is supplied with only three
columns of data, since the colour mappings are themselves multi-dimensional
when more columns are supplied. Issuing the command
...
column of data supplied to the \indpst{colourmap} plot style; further columns
