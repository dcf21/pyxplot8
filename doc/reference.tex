...
\begin{verbatim}
arc at <x>, <y> radius <r> from <start> to <finish> [with <option> ... ]
\end{verbatim}
...
specifies the co-ordinates of the centre of curvature, from which all points on
...
\begin{verbatim}
box at <x>, <y> width <w> height <h> [rotate <r>] [with <option> ... ]
box from <x1>, <y1> to <x2>, <y2> [rotate <r>] [with <option> ... ]
\end{verbatim}

There are two different ways to use the {\tt box} command to specify boxes that
are to be drawn onto a multi-plot.  In the first syntax, the co-ordinates of one
corner of the box are specified, along with the width and height.  If both width
and height are positive then the co-ordinates are taken to be those of the
...
\noindent The second syntax allows two pairs of co-ordinates to be specified.
...
co-ordinates of the circle's centre and its radius are specified.  Additional
...
co-ordinates.  It is equivalent to plotting a \datafile\ continaing a single
